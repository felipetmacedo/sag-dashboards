...
export default function RelatoriosContainer() {
	...
	const dailySalesReport = useMemo(() => {
		...
		propostasRaw.forEach((proposta: Proposta) => {
			...
			try {
				...
				if (
					(isAfter(propostaDate, startOfMonth(currentDate)) ||
						isEqual(propostaDate, startOfMonth(currentDate))) &&
					(isBefore(propostaDate, endOfMonth(currentDate)) ||
						isEqual(propostaDate, endOfMonth(currentDate)))
				) {
		...
		result.forEach((day) => {
			if (day.quantidade > 0) {
				...
				day.mediaAcumulada = parseFloat(
					(accumulatedSales / dayCount).toFixed(2)
				);
			} else {
				day.acumulado = accumulatedSales;
				day.mediaAcumulada =
					dayCount > 0
						? parseFloat((accumulatedSales / dayCount).toFixed(2))
						: 0;
			}
		});
		...
		propostasRaw.forEach((proposta: Proposta) => {
			...
			try {
				...
				if (
					...
					const vendorName =
						proposta.NOME_VENDEDOR || 'Não informado';
					...
					salesMap[vendorName][dateStr] =
						(salesMap[vendorName][dateStr] || 0) + 1;
				}
			} catch {
				console.error('Error parsing date:', proposta.DT_BORDERO);
			}
		...
		propostasRaw.forEach((proposta: Proposta) => {
			...
			try {
				...
				if (propostaDate.getFullYear() === currentYear) {
					...
					salesMap[vendorName][monthNumber] =
						(salesMap[vendorName][monthNumber] || 0) + 1;
				}
			} catch {
				console.error('Error parsing date:', proposta.DT_BORDERO);
			}
	...
	const dailySalesByVendorHeaders = useMemo(() => {
		if (
			...
			dailySalesByVendorReport.length === 0
		) {
	...
	const monthlySalesByVendorHeaders = useMemo(() => {
		if (
			...
			monthlySalesByVendorReport.length === 0
		) {
			return [{ label: 'Vendedor', key: 'vendedor' }];
		}
